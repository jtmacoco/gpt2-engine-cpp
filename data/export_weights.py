...
with open("gpt2_embeddings.bin", "wb") as f:
    ...
    for layer_idx in range(12):
        #Layer norm 1
        ln_1_g = state_dict[f'transformer.h.{layer_idx}.ln_1.weight'].detach().numpy()
        ln_1_b = state_dict[f'transformer.h.{layer_idx}.ln_1.bias'].detach().numpy()
        ...
        proj_w.tofile(f)
        proj_b.tofile(f)
    ln_f_g = state_dict['transformer.ln_f.weight'].detach().numpy()
    ln_f_b = state_dict['transformer.ln_f.bias'].detach().numpy()
    ln_f_g.tofile(f)
    ln_f_b.tofile(f)
